...
Per this workspace's established process (see Milestone 1's, 2's, and 3's own closing notes), a final review ran across the whole branch's diff before merging to `main`. The list below is that review's ACCURATE, POST-IMPLEMENTATION state — it supersedes everything in this section as originally drafted (kept below, struck through in spirit, for history): the `NetworkService`/`TcpLoopbackNetworkService` reconciliation this section originally called for already happened (the `## Amendment: real Milestone 3 Transport integration` section above), and several of the amendment-era predictions turned out to need small corrections once real E2E scenarios actually ran.

**Fixed during the final review (not deferred):**
- `liveSpecClient.ts` never tracked the live-spec `version` it was applying patches against, so a patch missed during the brief IPC round-trip between `open_conversation` resolving and `listen(...)` registering would silently, permanently desync the rendered transcript from backend state (storage itself stayed correct) — with `resync_conversation` and the 8-version rolling window never actually reachable in the shipped app despite existing and being unit-tested. Fixed by tracking version client-side and calling `resync_conversation` on any mismatch.
- `SPACECHAT_ACTOR_NAME`'s deterministic-device-id override was the one test-only env var not also gated behind `SPACECHAT_TEST_HOOKS` (all three test-only commands already were) — closed for consistency.

**Still accurate, deliberately deferred (not fixed in this milestone):**
- **Replace `PlaintextMembership` (Task 3) and the invite flow (Task 12) once `space-chat-openmls` exists.** Nothing in this plan should be mistaken for real security — unsigned invite tokens, unauthenticated membership, no cross-device gossip of membership changes at all. `join_via_invite` in particular looks like a working invite system from its command surface and is not one (no signature, no verification) — currently harmless only because no UI calls it.
- **`ObservedAtStore`'s "observed at" vs. "sent at" distinction (Task 4)** is a real, documented limitation, not a bug — `relative_time` can be inaccurate for a message received long after it was sent. Worth a genuine sender-side timestamp on `space_chat_core::domain::Message` once `space-chat-openmls`/a real wire format exist.
- **`CURRENT_EPOCH` is hardcoded to `0`** throughout `commands.rs` — no epoch rollover (MLS re-keying) implemented at all. Fine for a single long-lived epoch; a real gap once `space-chat-openmls` introduces actual epoch transitions.
- **Title persistence doesn't exist.** Task 9's fix made the *active-conversation* title correctly flow from what `open_conversation` was called with (no longer silently falling back to `space_id` on every regeneration, as an earlier draft of this note said) — but nothing persists it across a restart, and `App.tsx` still hardcodes `"General"` as the only title it ever opens with.

**New, discovered only once the real E2E harness (Tasks 17-20) actually ran — not visible to any task-scoped review, and not in this note's original draft:**
- **`TransportIdentity` is regenerated on every launch and never persisted (`lib.rs`'s own TODO).** This is the single largest gap between "the E2E harness proves it works" and "this is a usable P2P app": no peer can ever re-find this app across a restart in real use. The E2E harness itself only works because every actor re-publishes and re-dials fresh on every launch, matching the harness's own topology each time — this doesn't generalize to two real users' devices reconnecting after either one restarts. **This should be the first thing the next milestone fixes.**
- **Single-space by construction.** `DEFAULT_SPACE_ID` is the only space the shipped frontend ever opens. `create_space`/`join_via_invite` mint or join space ids the UI can never display or switch to, and `join_via_invite_impl` never calls `segment_arc`, so a joined space is never even registered with `Transport`. A real multi-space app needs a space list UI and the "register every space before connecting" story extended beyond one hardcoded id.
- **`withGlobalTauri: true` now ships in production**, because the E2E harness's test-only commands need `window.__TAURI__.core.invoke` reachable from a WebDriver-injected script with no access to the app's own bundled JS. Verified non-authorization-bearing on its own (Tauri's `invoke` bridge was already unconditionally injected regardless of this setting; the ACL/capabilities system, unchanged, is what actually gates commands) — but combined with this app's already-disabled CSP (`"csp": null`) and the three runtime-gated (not compile-time-gated) test-only commands sitting in the production command list, the WHOLE-APP security posture is worse than any one piece alone: a future XSS would have an easier path to the full `invoke` surface than in an app with either mitigation in place. No live exploit vector was found in this review (message content, sender names, and attachment URLs are all either auto-escaped or hash-derived, never raw peer text rendered as markup) — this is a posture concern to revisit, not a bug to fix now.
- **Two devices can render the same transcript in different orders.** `state.rs`'s `append_new_listing_entries` (network-synced messages) has no way to recover true chronological order for multiple messages arriving in one sync batch (`Segment::message_keys()` has no guaranteed order) — documented in code, but user-visible enough to belong in this note too.
- **The cucumber/fantoccini E2E harness is Linux-only in practice, not just "Linux/Windows as of implementation time."** It hardcodes `/usr/bin/WebKitWebDriver` and shells out to `xvfb-run` — running it on Windows would need real path/tooling changes beyond just `tauri-driver` itself being available there, and macOS remains ungated by Tauri's own `tauri-driver` support as originally noted.
- **No UI exists yet for several already-built commands**: `react`, `delete_message`, `fetch_older_page` (`has_more_older` is in every spec but nothing renders a "load more" affordance), `resync_conversation`, `generate_invite`, `join_via_invite`. All are real, tested Rust commands with no frontend caller.
- **Peer display names never propagate outside the test harness.** Outside `seed_membership_for_testing`, nothing gossips a device's display name to peers — a real second device shows every remote sender as an 8-character hex prefix, and the local device always shows as "Me".
- Several small, independently-Minor items recorded in the final review's own findings (a `Box::leak` per attachment request, `membership.json` growing by one orphaned space per app launch, two hex-decoding helpers sharing `decode_hash_hex`'s already-fixed char-boundary-panic risk, env-var gates checking presence rather than value, `active`'s lock held across up to 50 sequential redb write transactions on a page's first view, an on-disk cursor that can benignly regress by one step, and a narrow crash window that can orphan one message from the listing index until the next sync heals it) — none blocking, all worth a look in a housekeeping pass.
