...
> **Amendment (post-review):** two findings on the original version of this task:
> 1. The `attachment_survives_the_grace_window...` test's `live` set stayed non-empty (`HashSet::from([hash])`) all the way through `t_20_days`, only going empty at that same instant -- so `first_seen_unreferenced` started at `t_20_days`, not at `t0`, and the final assertion was checking `elapsed == 0 < grace_window`, trivially true regardless of whether 20-elapsed-days-of-being-unreferenced logic works at all. This didn't reproduce the storage spec's own Gherkin scenario (unreferenced starting at day 0, still present after 20 *elapsed* days of that). Fixed below: `live` goes empty starting at `t0` (modeling "Alice deletes the message" as an immediate local liveness change on the device running the sweep -- per the spec, GC is inherently per-device; a *different* device (Bob) still holding a reference is exactly the reason the grace window exists, not something this device's own `sweep` call needs to simulate directly), and the `t_20_days` check now genuinely exercises "20 days elapsed since first-unreferenced, still present."
> 2. The test file imported `ListingEntry`/`ListingIndex`/`TantivySearchIndex` but never called `append_entry`/`page`/`index_message`/`search` on any of them, despite this crate's own dev-dependency rationale ("to exercise all three [storage backends] together"). Fixed below by extending the convergence test to also drive a `RedbListingIndex` and a `TantivySearchIndex` off the same converged messages, closing the gap rather than just deleting the now-provably-relevant imports.
...
### Resolution: final whole-branch review findings (post-Task-8)

The final review found one Critical and several Important issues, all fixed directly (not via a new numbered task, since they cut across Tasks 1/2/4/6/7/8 and the already-merged Milestone 1 `Projection` trait):

- **Critical, fixed:** `Projection::watermark()` was a single unscoped scalar, so `catch_up` (and every real `Projection` impl: `RedbListingIndex`, `TantivySearchIndex`) silently conflated every space and epoch sharing one instance -- a later epoch's `Segment` restarts its own `cursor` counter from 0 (per `Segment::new`), so its `SegmentChange.cursor` could be lower than an earlier epoch's already-applied watermark and get skipped as "already caught up." No test in the milestone used more than one epoch, which is why this went uncaught through eight task reviews. Fixed by changing `Projection::watermark(&self) -> SegmentCursor` to `Projection::watermark(&self, space_id: &str, epoch: u64) -> SegmentCursor`, scoped per-key in every implementor (`space-chat-core/src/projection.rs`'s and `replay.rs`'s test fakes, `RedbListingIndex`, `TantivySearchIndex`). Regression tests added at every layer proving a numerically-lower later epoch/space is tracked independently, not conflated.
- **Important, fixed:** `TantivySearchIndex::index_message` had no upsert semantics -- combined with its in-memory-only watermark, a restart would replay from cursor 0 and duplicate every message in the index. Fixed via `writer.delete_term` before `add_document`.
- **Important, fixed:** `space_id` flowed unvalidated into `FileSegmentStore`'s filesystem paths -- a `space_id` containing `..` or a path separator (which, in this p2p app, could originate from a remote invite) was a real path-traversal risk. Fixed via `validate_space_id`.
- **Important, fixed:** `RedbAttachmentMetadataStore`'s hand-rolled `decode` indexed bytes unchecked and panicked on any short/corrupt row; `encode` `debug_assert!`ed on an over-long mime then cast to `u16` anyway, silently wrapping in release builds. Both now return `Result<_, StorageError>` with real bounds checks.
- **Important, fixed (cheap, folded in):** `RedbListingIndex::page` materialized an entire space's history before truncating to `limit` -- switched to `.rev().take(limit)` on the (already `DoubleEndedIterator`) redb range. `RedbAttachmentMetadataStore::clear_unreferenced` unconditionally wrote+committed for every live hash on every sweep even when nothing changed -- added the same already-clear short-circuit `mark_unreferenced_if_unset` already had.
- **Deferred, explicitly (not silently dropped):** non-atomic writes in `FileSegmentStore`/`FileAttachmentStore` (`fs::write` with no temp-file+rename, no checksum) against the storage spec's stated error-detection mechanism ("detected via checksum/hash mismatch on the segment file"). This is real design work (a file-format decision), not a quick fix, and the reviewer explicitly allowed treating it as a tracked follow-up rather than a merge blocker. **Whoever picks up Milestone 4's composition root, or a dedicated hardening pass before shipping real user data, needs to design this** -- segment and attachment bytes are two of the three things this plan's own Global Constraints call irreplaceable, and a crash mid-write currently produces a truncated file with no detection beyond a bare length check.

Also folded in the two Minor findings from Task 8's own review that were about to be superseded anyway: the GC test now proves genuine deletion (not just "still present"), and the convergence test now exercises `ListingIndex`/`SearchIndex` for real instead of leaving their imports unused.
